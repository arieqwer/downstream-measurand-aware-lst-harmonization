#!/usr/bin/env python3
"""Independently validate the retained reproducibility package.
...
Examples
--------
Run inside the self-contained repository::
...
SAMPLES = (
    Sample(
        ...
        "outputs/remote_sensing_validation/goes17_goes16_external_metric/"
    ...
    Sample(
        ...
        "outputs/remote_sensing_validation/multiplatform_expanded_validation/"
    ...
    Sample(
        ...
        "outputs/remote_sensing_validation/multiplatform_harmonization/"
    ...
    Sample(
        ...
        "outputs/remote_sensing_validation/goes18_goes19_metric_harmonization/"
...
def validate_bootstrap_archive(
    ...
    for _, row in summary.iterrows():
        ...
        scope_rows = draws.loc[draws["scope"].eq(scope)]
        if metric == "contrast_rmse_reduction_fraction" and scope == "hourly":
            values = 1.0 - np.sqrt(
                scope_rows["harm_downstream_mse_k2"]
                / scope_rows["raw_downstream_mse_k2"]
            )
        elif metric in draws.columns:
            values = scope_rows[metric]
        else:
...
def source_candidates(
    ...
    candidates = [root / logical, source_root / logical]
    if root_label in {"reproducibility_repository", "retained_source_tree"}:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Reproducibility-package root (default: inferred from this script).",
    )
    parser.add_argument(
        "--source-root",
        type=Path,
        ...
        help=(
            ...
            "are copied into the reproducibility package."
        ),
    )
