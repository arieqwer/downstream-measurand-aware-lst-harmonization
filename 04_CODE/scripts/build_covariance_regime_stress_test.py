...
def build_figure(regimes: pd.DataFrame, empirical: pd.DataFrame) -> None:
    plt.rcParams.update(
        {
            ...
            "pdf.fonttype": 42,
            "ps.fonttype": 42,
            "svg.hashsalt": "remote-sensing-covariance-regime-v1",
        }
    )
    # The 8.2-inch source is placed at 6.879 inches in the manuscript
    # (scale 0.839). Source fonts are enlarged accordingly so final effective
    # typography remains at least 8 pt while retaining room for annotations.
    fig = plt.figure(figsize=(8.2, 3.8))
    # Fixed normalized positions keep the colorbar close to panel a while
    # reserving enough inter-panel space for both vertical axis labels.
    axes = [
        fig.add_axes([0.075, 0.17, 0.35, 0.755]),
        fig.add_axes([0.635, 0.17, 0.35, 0.755]),
    ]
    colorbar_axis = fig.add_axes([0.442, 0.17, 0.015, 0.755])
    ...
    gain_loss_diverging = LinearSegmentedColormap.from_list(
        "gain_loss_diverging",
        [
            (0.0, "#D37886"),
            (0.2, "#EEC0C7"),
    ...
    colorbar.set_label(r"Deterministic proportion with $G_D>0$", fontsize=10.7)
    colorbar.ax.tick_params(labelsize=9.6, direction="out", length=3, width=0.7)
    ...
    annotation_positions = {
        "2021 external GOES17/16": ((2.35, 2.18), "right"),
        "2022-2024 expansion GOES18/16": ((0.03, 1.64), "left"),
        "2025 original GOES18/19": ((2.66, 1.43), "right"),
        "2025 expansion GOES18/19": ((0.38, 2.08), "left"),
        "2026 combined GOES18/19": ((1.70, 1.23), "center"),
    }
    for row in empirical.itertuples(index=False):
        improved = row.net_downstream_mse_gain_k2 > 0
        if improved:
        ...
        label_position, horizontal_alignment = annotation_positions[row.sample]
        axes[1].annotate(
    ...
    fig.savefig(
        FIGURES / "Figure_4_covariance_regime_stress_test.png",
        dpi=600,
...
def write_artifact_hashes() -> None:
    paths = [
        ...
        FIGURES / "Figure_4_covariance_regime_stress_test.png",
        FIGURES / "Figure_4_covariance_regime_stress_test_caption.md",
    ]
    hashes = {
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Rebuild and validate deterministic evidence without replacing the approved Figure 4 PNG.",
    )
    ...
    approved_figure = FIGURES / "Figure_4_covariance_regime_stress_test.png"
    if args.preserve_approved_figure:
        if not approved_figure.is_file():
            raise FileNotFoundError(approved_figure)
    else:
