...
package-local retained source tree. It never modifies that source tree. All derived
tables are written to ``02_EVIDENCE/si_tables`` and byte-for-byte copies of the
relevant frozen manifests are written to ``03_PROTOCOLS/frozen_inputs``.
"""
...
DEFAULT_SOURCE_ROOT = MANUSCRIPT_ROOT
DEFAULT_NAME_LOOKUP = (
    MANUSCRIPT_ROOT
    / "03_PROTOCOLS/frozen_inputs/AUXILIARY_UCDB_94_CITY_LOOKUP.csv"
)
DEFAULT_OUTPUT_DIR = MANUSCRIPT_ROOT / "02_EVIDENCE/si_tables"
DEFAULT_FROZEN_INPUT_DIR = MANUSCRIPT_ROOT / "03_PROTOCOLS/frozen_inputs"
...
STAGES = (
    Stage(
        ...
        51,
        31,
        "outputs/remote_sensing_validation/multiplatform_harmonization/derivation_city_event_hourly.parquet",
    ),
    Stage(
        ...
        43,
        31,
        "outputs/remote_sensing_validation/multiplatform_expanded_validation/historical_expanded_predictions.parquet",
    ),
    Stage(
        ...
        51,
        10,
        "outputs/remote_sensing_validation/multiplatform_harmonization/validation_2025_predictions.parquet",
    ),
    Stage(
        ...
        43,
        10,
        "outputs/remote_sensing_validation/multiplatform_expanded_validation/validation_expanded_predictions.parquet",
    ),
    Stage(
        ...
        94,
        10,
        "outputs/remote_sensing_validation/multiplatform_expanded_validation/combined_2025_common_domain_predictions.parquet",
    ),
    Stage(
        ...
        94,
        8,
        "outputs/remote_sensing_validation/goes18_goes19_metric_harmonization/validation_2026_hourly_predictions.parquet",
        "outputs/remote_sensing_validation/goes18_goes19_metric_harmonization/validation_2026_transition_predictions.parquet",
    ),
    Stage(
    ...
    Stage(
        ...
        75,
        4,
        "outputs/remote_sensing_validation/goes17_goes16_external_metric/validation_2021_hourly_predictions.parquet",
        "outputs/remote_sensing_validation/goes17_goes16_external_metric/validation_2021_transition_predictions.parquet",
...
FROZEN_COPIES = {
    ...
    "INITIAL_HARMONIZATION_FROZEN_MANIFEST.json": (
        "outputs/remote_sensing_validation/multiplatform_harmonization/frozen_harmonization_manifest.json"
    ),
    "METRIC_AWARE_HARMONIZATION_FROZEN_MANIFEST.json": (
        "outputs/remote_sensing_validation/goes18_goes19_metric_harmonization/frozen_metric_aware_manifest.json"
    ),
}
...
def build_attrition_summary(source_root: Path) -> pd.DataFrame:
    external_report_path = source_root / "outputs/remote_sensing_validation/goes17_goes16_external_metric/external_replication_report.json"
...
def build_event_counts(source_root: Path, chronology: pd.DataFrame) -> pd.DataFrame:
    ...
    external_attrition = pd.read_csv(
        source_root / "outputs/remote_sensing_validation/goes17_goes16_external_metric/event_attrition.csv"
    )
...
def build_model_selection(source_root: Path) -> pd.DataFrame:
    initial = pd.read_csv(
        source_root / "outputs/remote_sensing_validation/multiplatform_harmonization/derivation_model_selection.csv"
    ).rename(
        columns={
    ...
    metric = pd.read_csv(
        source_root / "outputs/remote_sensing_validation/goes18_goes19_metric_harmonization/model_selection.csv"
    ).rename(
        columns={
...
def build_uncertainty_gate(source_root: Path, chronology: pd.DataFrame) -> pd.DataFrame:
    prediction_path = source_root / "outputs/remote_sensing_validation/goes18_goes19_metric_harmonization/validation_2026_transition_predictions.parquet"
    ...
    frozen = pd.read_csv(
        source_root / "outputs/remote_sensing_validation/goes18_goes19_metric_harmonization/validation_2026_uncertainty.csv"
    ).set_index("cohort")
...
def source_inventory(source_root: Path, name_lookup: Path) -> list[Source]:
    source_root_label = (
        ...
        if source_root == MANUSCRIPT_ROOT
        else "retained_source_tree"
    )
    entries = [
        ...
        ("selection_initial", "outputs/remote_sensing_validation/multiplatform_harmonization/derivation_model_selection.csv", "initial model-selection scores"),
        ("selection_metric_aware", "outputs/remote_sensing_validation/goes18_goes19_metric_harmonization/model_selection.csv", "metric-specific model-selection scores"),
        ("uncertainty_frozen_summary", "outputs/remote_sensing_validation/goes18_goes19_metric_harmonization/validation_2026_uncertainty.csv", "frozen 2026 uncertainty summary"),
        ("uncertainty_predictions", "outputs/remote_sensing_validation/goes18_goes19_metric_harmonization/validation_2026_transition_predictions.parquet", "row-level 2026 uncertainty outcomes"),
        ("external_offsets", "outputs/remote_sensing_validation/goes17_goes16_external_metric/frozen_hour_offsets_2019_2020.csv", "frozen external hour offsets"),
        ("external_attrition", "outputs/remote_sensing_validation/goes17_goes16_external_metric/event_attrition.csv", "external event-level attrition"),
        ("external_report", "outputs/remote_sensing_validation/goes17_goes16_external_metric/external_replication_report.json", "external aggregate analysis counts"),
...
def main() -> None:
    ...
    external_offset_source = (
        source_root
        / "outputs/remote_sensing_validation/goes17_goes16_external_metric/frozen_hour_offsets_2019_2020.csv"
    )
